    ...
    for idx, batch in enumerate(data):
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                outputs = model(input_ids=batch[0], attention_mask=batch[1], normalize=False, output_hidden_states=True)
...
class IsotropyEvaluator:
    ...
    def iso_score_star(self, sentence_model, ddp, general=False, x="passage", y="passage", n_evals=1500, batch_size=100, zeta=0.2):
        #model.train()
        random.seed(100)
        if general:
        ...
        texts = sentence_model.tokenizer(samples, return_tensors="pt", padding=True, truncation=True)
        ...
        h = 768
        reg = istar()
        C0 = compute_shrinkage_matrix(dataloader, ddp)
        isos = torch.tensor(0, dtype=torch.float64,device=torch.device("cuda"))

        for idx, batch in enumerate(dataloader):
            with torch.no_grad():
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    outputs = ddp(input_ids=batch[0], attention_mask=batch[1], normalize=False, output_hidden_states=True)
                
                points = torch.reshape(outputs["hidden_states"], (-1,h)).to(torch.float32)
...
class MultiConceptIsotropyEvaluator:
    ...
    def evaluate(self, ddp, sentence_model):
        results = {}
        # Evaluate using the general evaluator
        results["general_cosreg"] = self.general_eval.gaslite_cosreg(sentence_model, general=True)
        results["general_isoscore"] = self.general_eval.iso_score_star(sentence_model, ddp, general=True)
        #results["general_isoscore"] = None

        # Evaluate the concept-specific metrics
        for concept in self.concepts_to_eval:
            results[f"{concept}_cosreg"] = self.concept_evals[concept].gaslite_cosreg(sentence_model)
            results[f"{concept}_isoscore"] = self.concept_evals[concept].iso_score_star(sentence_model, ddp)
