...
from contrastors.models.encoder.modeling_nomic_bert import NomicBertPreTrainedModel, NomicBertModel
...
def save_beir_eval(
    ...
    out_file = os.path.join(out_dir, _get_cached_eval_name(dataset_name, data_split,
                                                           data_portion, model_hf_name, sim_func_name) + "new")
    ...
    if model_hf_name in TRUST_CODE_MODELS:
        config_model = SentenceTransformer(model_hf_name, trust_remote_code=True)
        model = models.SentenceBERT()
        sent_model = SentenceTransformer(model_hf_name, trust_remote_code=True)
        sent_model[0].auto_model = NomicBertModel.from_pretrained(f"/home/sharifm/students/yorayh/tmp/vaktibabat/contrastors/src/contrastors/ckpts/nomic-embed-text-v1-robust/step_100/model/", config=config_model[0].auto_model.config).to(torch.device("cuda"))
...
def load_cached_eval(
    ...
    #    out_dir = os.path.join(os.getcwd(), "data", "cached_evals")
    #    os.makedirs(out_dir, exist_ok=True)
    #    local_results_path = os.path.join(out_dir, _get_cached_eval_name(dataset_name, data_split, data_portion, model_hf_name, sim_func_name))

    local_results_path = "./data/cached_evals/msmarco-train-concepts_1.0_nomic-embed-text-v1_cos_sim.json"
